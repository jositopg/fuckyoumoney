-- Copia de mezcla: 80.9, no 80.9000000000000000. Pegar ENTERO y Run.

CREATE OR REPLACE FUNCTION public.fym_num(n numeric)
RETURNS text
LANGUAGE sql
IMMUTABLE
PARALLEL SAFE
AS $$
  SELECT CASE WHEN n IS NULL THEN NULL ELSE trim_scale(round(n, 1))::text END;
$$;
...
BEGIN
  ...
  ELSIF expenses <= 0 THEN
    mix_stance := 'unknown';
    mix_headline := format('El ladrillo es el %s%% del patrimonio.', public.fym_num(mix_re_pct));
    mix_detail := format('Efectivo %s%% · fondos %s%%. Sin tu gasto no se sabe si el efectivo aguanta un parón. No vendas hasta saberlo.', public.fym_num(mix_cash_pct), public.fym_num(mix_invested_pct));
  ELSIF mix_re_pct < 60 THEN
    mix_stance := 'ok';
    mix_headline := format('La mezcla no está atrapada en ladrillo (%s%%).', public.fym_num(mix_re_pct));
    mix_detail := CASE WHEN mix_shock IS NOT NULL THEN format('Si el alquiler para, el efectivo cubre %s meses.', public.fym_num(mix_shock)) ELSE 'Hay margen fuera del inmueble.' END;
  ELSIF mix_shock IS NOT NULL AND mix_shock < 6 THEN
    ...
    mix_detail := format('Ladrillo %s%% · efectivo %s%%. Equilibrar es vender (vacíos), no aportar a fondos.', public.fym_num(mix_re_pct), public.fym_num(mix_cash_pct));
  ELSIF mix_shock IS NOT NULL AND mix_shock < mix_shock_target AND COALESCE(re.vacant, 0) >= 2 THEN
    mix_stance := 'divest_brick';
    mix_headline := format('El ladrillo pesa %s%% y hay %s vacíos.', public.fym_num(mix_re_pct), re.vacant);
    mix_detail := format('Si el alquiler para, el efectivo cubre %s meses. Los vacíos son los candidatos a vender.', public.fym_num(mix_shock));
  ELSIF mix_shock IS NOT NULL AND mix_shock >= 6 AND mix_invested_pct < 15 THEN
    mix_stance := 'rebalance_with_cash';
    mix_headline := format('El ladrillo pesa %s%%, pero no hace falta vender.', public.fym_num(mix_re_pct));
    mix_detail := format('Si el alquiler para, el efectivo cubre %s meses. Equilibra con fondos, no deshaciendo pisos.', public.fym_num(mix_shock));
  ELSE
    mix_stance := 'ok';
    mix_headline := format('Ladrillo %s%%, y hay colchón de golpe.', public.fym_num(mix_re_pct));
    mix_detail := format('Fondos %s%% · efectivo %s%%.', public.fym_num(mix_invested_pct), public.fym_num(mix_cash_pct));
  END IF;
